import { DECK_OPTIONS, QUIZ_BANKS } from '../src/content/quiz-banks.js';
import { buildSessionQuestions, nextDueLabel, selectSessionQuestions } from '../src/lib/quiz-bank.js';
import { isDue, newCardSchedule, scheduleReview } from '../src/lib/spaced-repetition.js';

// Exercises the bundled quiz banks and the spaced-repetition layer (scheduler
// plus session builder) in plain node: pure logic, no server, no secrets.
// Exits 1 if any case fails. Run from the frontend folder with
...
async function main() {
  ...
  const beforeHard = { ...card, intervalDays: 10, repetitions: 3 };
  ...
  for (const label of ['single chapter', 'mixed chapters']) {
