...
`/api/chat` POST takes `{ message, materialId?, history }` where `history` is the caller's last 10 messages (`{role, content}`), and answers `{ reply, suggestions }` with 0 to 3 short follow-up suggestions. It counts one AI call. The server appends the user message and the reply to the profile's stored history (last 50 messages kept). GET returns `{ messages }`. DELETE clears the profile's stored history and answers `{ messages: [] }`. Storage is the Firebase Realtime Database when both the service account and `FIREBASE_DATABASE_URL` are set, the in-memory store otherwise; the browser never talks to Firebase.

The quiz responses (`/api/quiz` and `/api/quizzes/[id]`) carry the answer key per question (`correctAnswerIndex` and `explanation` in each public question) from the UI port on: the Quiz screen's "Check Answer" step reveals them client-side. Grading stays server-side and authoritative in `/api/attempt`. There are no accounts and no stakes, so exposing the key to the browser costs nothing. `difficulty` (already in `QuizRequest`) is honoured: the Gemini prompt pitches the questions at the requested level; the mock treats it as cosmetic.
...
- `checkIpLimit`: 60 requests per minute per IP by default, overridable through the optional `IP_REQUESTS_PER_MINUTE` env variable (bounds 1 to 100000, invalid values fall back to 60). The robot e2e suite raises it to 1000 for the server it starts, because a full run makes roughly 65 profiled requests inside one minute from one IP. Rejected requests are logged to the server console as a warning before the 429 goes back.
...
Optional tuning variables: `DAILY_AI_CALL_CAP`, `IP_REQUESTS_PER_MINUTE` (section 6), `GEMINI_MODEL`, and `FIREBASE_DATABASE_URL`, which activates Realtime Database chat history when the service account is also set (section 4).
