...
- UI port screen map (foundation and backend landed 2026-09-05; screens next): `/` is the static Landing; `/dashboard` renders their dashboard (greeting, streak computed client-side from progress last-practised dates, stat cards from `/api/me`, Quick Actions, Review today with "Study this topic" linking to `/progress?material=<id>&topic=<id>`, Recent Activity, and the "Try sample notes" and "Upload notes" actions); `/notes` is the Notes Generator (their upload zone plus the paste box, browser PDF extraction and the scanned-PDF fallback, then their result tabs Notes / Flashcards / Key points from `/api/notes`, the topic list with mastery bars, and Recent Study Materials); `/quiz` is the Quiz Arena (start screen with the active material, difficulty and question count, their question flow with Check Answer revealing the answer key the quiz response now carries, submit through `/api/attempt`, and their results view with the score circle, per-topic results, "Study weak topics" linking to `/progress` and "Practise weak topics" restarting with `focusWeak`); `/progress` is their Progress page (stat cards, the recharts performance trend over past attempts, Recent Quizzes, and a Focus Areas section: weak topics with mastery bars and expandable explanations from `/api/explain`, plus the study feedback card with its refresh); `/chat` is their AI Tutor on `/api/chat` (history loaded on mount, suggestions, Clear calling DELETE); `/teacher-dashboard` and `/admin-dashboard` are the static demo ports. The active material is shared through `src/lib/active-material.ts` (localStorage `edubuddy.activeMaterialId`, set by `/notes`, read by `/quiz` and `/chat`). Old routes (`/upload`, `/notes/[id]`, `/notes/[id]/quiz`, `/notes/[id]/results`, `/study/[materialId]/[topicId]`) and their now-unused components, plus the dev preview page and its fixtures, are recycled in a cleanup pass once the new screens and Robot suites are green.
